...
library(janitor)
library(lubridate)
Harry_2018_data <- read_csv("data/md_gov_county_18.csv") |> clean_names()
Cox_2022_data <- read_csv("data/md_gov_county.csv") |> clean_names()
```
...
**A1.** https://datawrapper.dwcdn.net/jvVvW/1/

```{r}
Cox_data <- Cox_2022_data |> mutate(total_votes = cox + moore + lashar + wallace + harding + sefcik + write_ins) |> mutate(pct_cox = (cox/total_votes)*100)

Hogan_data <- Harry_2018_data |> mutate(total_votes = hogan + jealous + quinn + schlakman + write_ins) |> mutate(pct_hogan = (hogan/total_votes)*100)

Fused_data<- Hogan_data |> left_join(Cox_data, by="geoid")

write_csv(Fused_data, "data/md_gov_county_18.csv")
```
