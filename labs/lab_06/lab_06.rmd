...
Allegany_data <- read_csv("data/allegany_911.csv")
Baltimore_data  <- read_csv("data/baltimore_911.csv")
Carroll_data  <- read_csv("data/carroll_911.csv") |> clean_names()
Cecil_data  <- read_csv("data/cecil_911.csv")
...
Baltimore_data <- Baltimore_data |> mutate(across(zip_code, as.character))

Comeplete_Baltimore_data <- Baltimore_data |> left_join(Maryland_zcta, join_by(zip_code==zcta5n))
Counties_combined <- bind_rows(list(Allegany_data, Carroll_data, Cecil_data))
```
...
A1.Carroll has the most. Next is Cecil. Lastly, is Allegany.

```{r}
Ranked <- Counties_combined |> group_by(county) |> summarise(total=n()) |> arrange(desc(total))
```
