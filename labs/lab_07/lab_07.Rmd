...
**Q1** What is the Maryland jurisdiction with the lowest median age, according to the ACS 2023 5-year estimates? You will need to find the variable for median age first. **A1** Balitmore City.

```{r}
A1 <- get_acs(geography = "county", variables = c(medage = "B01002_001"), state = "MD", year = 2023) |> arrange(estimate)
```

**Q2** Which Maryland counties have a median age of at least 50? You can use the dataframe you just made to answer this, but you must write code to display only those counties with a median age of at least 50. **A2** There are only two counties with a median age of at least 50: Talbot and Worcester county.

```{r}
A2 <- get_acs(geography = "county", variables = c(medage = "B01002_001"), state = "MD", year = 2023) |> arrange(desc(estimate)) |> filter(estimate >= 50)
```
...
```{r}
montgomery_data <- read.csv("data/montgomery_2022_overdoses.csv") |> mutate(zip = as.character(zip)) |> group_by(zip) |> summarise(total_calls = n())
montgomery_data |> group_by(zip) |> summarise(n())
A3 <- get_acs(geography = "zcta", variables = c(population_18_and_up = "DP05_0021E"), state = "MD", year = 2019)
montgomery_data |> left_join(A3, by=c("zip" = "GEOID")) |> rename(population_18_and_up = estimate) |> mutate(call_rate = total_calls / population_18_and_up * 100000) |> arrange(desc(call_rate))
