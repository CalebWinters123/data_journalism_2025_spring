...
```{r} cleanest_maryland_expenses |> group_by(payee_clean, expense_purpose) |> summarise(count = n()) |> arrange(desc(count))}
```
...
**A4. Only Facebook spent spent all their onlie advertising dollars on themselves.**

```{r} Q4 <-cleanest_maryland_expenses |> group_by(payee_clean, expense_purpose) |> summarise(count = n()) |> arrange(desc(count))}
```
...
**A5 We know that Facebook paid for 1478 online advertisements. I wonder what these advertisements looked like.**
