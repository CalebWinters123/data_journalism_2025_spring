...
**A1** It is a tie between Summary Suspension Affirmed and Surrender of License. They both show up 7 times 

```{r}
Physician_Alerts <- "https://www.mbp.state.md.us/disciplinary_2024.aspx"

results <- Physician_Alerts |> read_html() |> html_table()

results <- results[[1]] |> clean_names()

results |> group_by(sanction) |> summarise(count=n()) |> arrange(desc(count))
```
...
```{r}
Press_release <- "https://www.opd.state.md.us/press-releases"

press_release <- Press_release |> read_html() |> html_elements('p.font_8.wixui-rich-text__text span a') |> html_text() |> as.tibble() |> separate(value, c('date', 'title'), sep=":") |> mutate(date = mdy(date))

press_release |> filter(str_detect(str_to_lower(title), "police"))
```
...
```{r}
Chris <- "https://www.vanhollen.senate.gov/news/press-releases"

title <- Chris |> read_html() |> html_element("h2.ArticleTitle") |> html_text()
