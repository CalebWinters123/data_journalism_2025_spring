...
A1. It is a tie between 2022-07-14 and 2022-10-04. After scrolling, I've noticed that there are some days missing.l

```{r}
Q1 <- cleanerest_Baltimore |> group_by(date) |> summarise(count = n()) |> arrange(desc(count))
```
...
A2. Most people are overdosing on weekends.

```{r}
Q2 |> group_by(day_of_week) |> summarise(count = n()) |> arrange(desc(count)) |> mutate(pct = count/ sum(count) * 100)
```
...
A3. 4540 SILVER SPRING RD, PC 02; 6424 WINDSOR MILL RD, and PC 06; 115 SUSQUEHANNA AV W have the most calls. A zipcode would be helpful for futher data analysis.

```{r}
Q3 <- cleanerest_Baltimore |> group_by(loc) |> summarise(count =n()) |> arrange(desc(count))

```

Q4. What's the best story idea or question you've seen as a result of the work you've done in this lab?

A4. What has overdoses looked like throughout the year of 2024?
