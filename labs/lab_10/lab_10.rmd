...
```{r}
Two_performances <- read_csv("data/md_statewide_county.csv") |> clean_names() |>  mutate(total_votes_gov = cox + moore + lashar + wallace + harding + sefcik + gov_writein) |> mutate(total_votes_ag = peroutka + brown + ag_writein) |> mutate(pct_moore = (moore/total_votes_gov)*100) |> mutate(pct_brown = (brown/total_votes_ag)*100) |> mutate(pct_dif = pct_moore - pct_brown) |> mutate(geoid = as.character(geoid))

md_counties <- counties(year=2021) |> clean_names() |> filter(statefp ==24) |> mutate(geoid = as.character(geoid))

Performances <- md_counties |> left_join(Two_performances, by="geoid")


ggplot() + geom_sf(data = Performances, aes(fill = pct_dif, geometry = geometry)) + scale_colour_viridis_b(option="magma") + theme_minimal()
