...
library(lubridate)
library(tidycensus)
pg_data<- read_csv("data/prince_georges_2022_overdoses.csv")
```
...
**A1.** The month with the highest amount of calls: December, and the month with the lowest: January do not have any major differences.

```{r}
pg_percentage_calls <- pg_data |> mutate(month = month(datetime, label=TRUE)) |> group_by(month) |> summarize(total_calls = n()) |> mutate(p_total_calls=total_calls/sum(total_calls)*100) |> arrange(desc(total_calls))
```

```{r}
pg_percentage_calls |> ggplot() + geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) + labs(title="Prince George's County Overdoses Calls in 2022", x = "Month", y = "Number of Calls", caption = "source: Prince George's County EMS")
```
...
**A2.** The graph spikes many times each week, but it falls in the month of May. I wonder why?

```{r}
 pg_data_week<- pg_data |> mutate(week = floor_date(datetime, unit = "week")) |> group_by(week) |> summarise(total_calls = n())
```

```{r}
 pg_data_week |> ggplot() + geom_line(aes(x=week, y=total_calls)) + labs(title="Prince George's County Overdoses Calls in 2022 by Week", x = "Month", y = "Number of Calls", caption = "source: Prince George's County EMS")
...
```{r}
pg_data_by_zip <- pg_data |> group_by(zipcode) |> summarize(total_calls = n()) |> arrange(desc(total_calls))
```
